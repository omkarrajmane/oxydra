...
async fn run() -> Result<(), VmError> {
        ...
        && telegram_config.enabled
    {
        ...
        {
            let proactive_sender = if let Some(ref updater) = bootstrap.delivery_streak_updater {
                Arc::new(
                    channels::telegram::TelegramProactiveSender::new_with_streak_updater(
                        &bot_token,
                ...
                Arc::new(channels::telegram::TelegramProactiveSender::new(
                    &bot_token,
                    telegram_config.max_message_length,
                ))
            };
