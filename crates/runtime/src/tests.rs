...
impl RecordingDelegationExecutor {
    fn new(runtime: Arc<AgentRuntime>, agents: BTreeMap<String, AgentDefinition>, root_selection: ProviderSelection) -> Self {
        Self {
...
async fn delegation_depth_spike_three_levels() {
    ...
    assert!(!records.is_empty(), "Expected at least one delegation record");
...
async fn delegation_depth_spike_session_id_chaining() {
    ...
    assert_eq!(depth_from_grandchild, 2, "Grandchild should have 2 subagent prefixes");
}
...
async fn delegation_depth_spike_cancellation_propagation() {
    ...
    assert!(result.is_ok() || result.is_err(), "Task should complete after cancellation");
}
...
async fn run_session_internal_enforces_deadline_before_provider_call() {
    ...
    assert!(
        result.is_err(),
        "Expected error when deadline is exceeded"
    );
}
...
async fn run_session_internal_allows_bounded_budget_overrun() {
    ...
    match result {
        Ok(response) => {
            ...
            assert_eq!(response.message.content.as_deref(), Some("response within bounds"));
        }
        Err(_) => {
