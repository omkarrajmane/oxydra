...
mod tests {
    ...
    fn budget_strategy() -> impl Strategy<Value = u64> {
        prop::num::u64::ANY.prop_filter("budget must be reasonable", |&v| v <= 1_000_000_000_000)
    }
    ...
    fn max_turns_strategy_policy() -> impl Strategy<Value = Option<u32>> {
        prop::option::of(
            prop::num::u32::ANY.prop_filter("turns must be reasonable", |&v| v <= 10000),
        )
    }

    /// Strategy for generating max_turns values (for AgentDefinition)
    fn max_turns_strategy_agent() -> impl Strategy<Value = Option<usize>> {
        prop::option::of(
            prop::num::usize::ANY.prop_filter("turns must be reasonable", |&v| v <= 10000),
        )
    }
    ...
    fn tool_name_strategy() -> impl Strategy<Value = String> {
        prop::collection::vec(
            prop::num::u8::ANY.prop_filter("valid ascii", |&v| (97..=122).contains(&v)),
            1..20,
        )
        .prop_map(|bytes| String::from_utf8(bytes).unwrap_or_else(|_| "tool".to_string()))
    }
    ...
    proptest! {
        ...
        fn prop_delegation_max_turns_narrowing_only(
            ...
            let request = DelegationRequest {
                ...
                caller_selection: None,
                key_facts: vec![],
                max_turns: child_turns.map(|v| v as u32),
                max_cost: None,
                parent_policy: None,
            };
