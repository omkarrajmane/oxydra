    ...
    async fn resolve_schedule_policy(
        ...
        let merged_per_run =
            RunPolicyInput {
                max_turns: per_run
                    .max_turns
                    .map(|t| t.min(self.config.max_turns as u32))
                    .or(Some(self.config.max_turns as u32)),
                max_budget_microusd: per_run.max_budget_microusd.or((self.config.max_cost > 0.0)
                    .then_some((self.config.max_cost * 1_000_000.0) as u64)),
        ...
        match policy_guard::resolve_policy(
            ...
            Err(e) => {
                ...
                let fallback_per_run = RunPolicyInput {
                    max_turns: Some(self.config.max_turns as u32),
