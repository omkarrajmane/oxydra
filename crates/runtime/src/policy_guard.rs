...
pub enum PolicyValidationError {
    ...
    /// Deadline is in the past (deadline <= started_at).
...
fn validate_effective_policy(policy: &EffectiveRunPolicy) -> Result<(), PolicyValidationError> {
    ...
    if policy.toolset.is_empty() {
        return Err(PolicyValidationError::EmptyToolset);
    }

    if policy.deadline.is_some_and(|d| d <= policy.started_at) {
        return Err(PolicyValidationError::PastDeadline);
    }
...
mod tests {
    ...
    #[test]
    fn edge_case_empty_toolset_after_filtering_rejected() {
        let global = create_runtime_config(50, None, 30);
        ...
        assert!(
            result.is_err(),
            "Empty toolset should be rejected at admission"
        );
        assert_eq!(result.unwrap_err(), PolicyValidationError::EmptyToolset);
    }
    ...
    fn edge_case_all_tools_disallowed_results_in_empty_toolset() {
        ...
        assert!(
            result.is_err(),
            "All tools disallowed should be rejected at admission"
        );
        assert_eq!(result.unwrap_err(), PolicyValidationError::EmptyToolset);
    }
    ...
    #[test]
    fn test_empty_toolset_rejected() {
        let global = create_runtime_config(50, None, 30);
        let agent = create_agent_definition(None, None, None);
        let tool_policy = create_tool_policy_input(
            None,
        ...
        assert!(result.is_err(), "Empty toolset should be rejected");
        assert_eq!(result.unwrap_err(), PolicyValidationError::EmptyToolset);
    }
    ...
    fn test_past_deadline_rejected() {
        ...
        let past_deadline = started_at - Duration::seconds(1);
        ...
        let result = validate_effective_policy(&policy);
